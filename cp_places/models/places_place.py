...
class PlaceReportRender(models.AbstractModel):
        # The name will be 'report.'+'module.'+'report template name'
    _name = 'report.cp_places.filter_place'

    @api.model
    def render_html(self, docids, data=None):
        """
        Overridden render_html method to print the report with external data
        --------------------------------------------------------------------
        @param self : object pointer
        @param docids : identifier(s) of the records
        @param data : data passed from the wizard if any
        """
